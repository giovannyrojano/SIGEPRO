...
module.exports = {
  register(name, lastName, email, password, role) {
    return new Promise(async (resolve, reject) => {
      ...
      if (verifyMail) {
        reject(new Error('este correo ya esta registrado'))
      } else {
        // encriptar la contraseña
        const passHash = CryptoJS.AES.encrypt(password, 'A&G').toString()
  ...
  login(email, password) {
    return new Promise(async (resolve, reject) => {
      ...
      if (verifyMail) {
        ...
        const decryptPassword = CryptoJS.AES.decrypt(verifyMail.password, 'A&G')
        ...
        if (utf8 !== password) {
          reject(new Error('email y/o contraseña incorrectos'))
        } else {
          // generacion del jwt de la sesion del usuario
          const token = jwt.sign(
            {
              id: verifyMail._id,
              nombre: verifyMail.name,
              apellido: verifyMail.lastName,
              rol: verifyMail.role,
            },
